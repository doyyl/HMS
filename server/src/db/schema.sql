...
-- ---------------------------------------------------------------------------
-- ---------------------------------------------------------------------------
-- Reservations (online / phone pre-bookings, prepaid)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS reservations (
  id              INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  code            TEXT NOT NULL UNIQUE,            -- guest-facing, e.g. 'R-260715-8K3D'
  room_id         INTEGER NOT NULL REFERENCES rooms(id),
  check_in_date   DATE NOT NULL,                   -- Asia/Bangkok calendar date
  check_out_date  DATE NOT NULL,                   -- exclusive; nights = diff
  nights          INTEGER NOT NULL,
  guest_name      TEXT NOT NULL,
  guest_phone     TEXT NOT NULL,
  guest_email     TEXT,
  amount          INTEGER NOT NULL,                -- integer baht, full prepay
  status          TEXT NOT NULL DEFAULT 'pending_payment'
                  CHECK (status IN ('pending_payment', 'confirmed', 'checked_in',
                                    'completed', 'cancelled', 'expired', 'no_show')),
  hold_expires_at TIMESTAMPTZ,                     -- now() + RESERVATION_HOLD_MINUTES
  booking_id      INTEGER REFERENCES bookings(id),
  manage_token    TEXT NOT NULL,                   -- random token for the guest status page
  source          TEXT NOT NULL DEFAULT 'online' CHECK (source IN ('online', 'phone', 'walkin')),
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- Payment-gateway (2C2P) records. invoice_no is the idempotency key.
...
CREATE INDEX IF NOT EXISTS idx_gateway_payments_reservation ON gateway_payments(reservation_id);

-- Allow prepaid online payments (from reservation check-in). Idempotent.
DO $$ BEGIN
  ALTER TABLE payments DROP CONSTRAINT IF EXISTS payments_method_check;
  ALTER TABLE payments ADD CONSTRAINT payments_method_check CHECK (method IN ('cash', 'qr', 'online'));
